...
**Status:** Design complete. Ready for content population and deployment.
...
### Hero composition rules (do not break these)

The hero uses a locked two zone absolute layout:
- **Logo zone:** top anchored at `6vh`, centered on the horizontal axis, max `44vh` tall
- **Content zone:** bottom anchored at `10vh`, centered, max 560px wide

The mic and CD are absolutely positioned decorative accents outside the content axis. They must never shift the perceived center of the headline.

The logo uses `mix-blend-mode: screen` with a radial feather mask. This is what eliminates the PNG bounding box. Do not remove either property.
